...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch-size", type=int, default=32, help="t5 batch size")
    parser.add_argument(
        "--model",
    ...
    if args.model == "google/t5-large-ssm":
        ...
        model = transformers.AutoModelForSeq2SeqLM.from_pretrained(args.model).to(
            device
        )
        remove_prefix = False
        ...
        tokenizer.mask_token = tokenizer.additional_special_tokens[0]
        model = transformers.T5ForConditionalGeneration.from_pretrained(args.model).to(
            device
        )
        remove_prefix = False
        ...
        model = transformers.BartForConditionalGeneration.from_pretrained(
            args.model, forced_bos_token_id=0
        ).to(device)
        remove_prefix = True
    else:
            ...
            for kind, template in templates.items():
                entry = {
                    ...
                    "text": template.format(
                        **{"mask": tokenizer.mask_token, attr_key: attr_value}
                    ),
                }
                formatted.append(entry)

        loader = data.DataLoader(formatted, batch_size=args.batch_size)  # type: ignore
        ...
        for batch in tqdm(loader, desc="filling contexts"):
            ...
            if remove_prefix:
                ...
                for ids, start in zip(outputs, starts):
                    completion = tokenizer.decode(ids[start:], skip_special_tokens=True)
                    completions.append(completion)
            else:
                completions = tokenizer.batch_decode(outputs, skip_special_tokens=True)

            for kind, attr_value, completion in zip(
                batch["kind"], batch[attr_key], completions
            ):
