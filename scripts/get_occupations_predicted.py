...
from src.utils import env, tokenizer_utils
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="get model-predicted occupations")
    parser.add_argument(
        "--discourse",
        action="store_true",
    ...
    samples = [
        ...
        {
            "prefix": entry["texts"]["prompt"]
            if args.discourse
            else f'{entry["entity"]} is most known for being a',
            "completions": [f"{occupation}." for occupation in occupations],
            "entity_token_range": tokenizer_utils.find_token_range(
                entry["texts"]["prompt"] if args.discourse else entry["entity"],
                entry["entity"],
                tokenizer,
    ...
    representations = torch.zeros(
        len(entries), num_layers + 1, model.config.hidden_size
    )
    results = []
    for index, sample in enumerate(tqdm(samples, desc="predict occupations")):
        ...
        with torch.inference_mode():
            outputs = model(
                inputs.input_ids, return_dict=True, output_hidden_states=True
            )

        past_key_values = tuple(
            tuple(k_or_v.expand(len(occupations), -1, -1, -1) for k_or_v in layer_kv)
        ...
        for layer in range(len(outputs.hidden_states)):
            representations[index, layer] = (
                outputs.hidden_states[layer][0, entity_tokens].mean(dim=0).cpu()
            )

        # Now try all the completions.
        loader = data.DataLoader(sample["completions"], batch_size=args.batch_size)
        ...
        scores = []
        for token_ids, logits in zip(chain(*batched_input_ids), chain(*batched_logits)):
