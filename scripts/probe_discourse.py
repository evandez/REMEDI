...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--k", type=int, default=5, help="record top-k probe predictions (default: 5)"
    )
    parser.add_argument(
    ...
    probe_results_dir = args.probe_results_dir or (
        env.results_dir() / "probe_occupations"
    )
    ...
    discourse_file = data_dir / args.model_key / "occupations-discourse-predicted.json"
    ...
    for layer in range(reps.shape[1]):
        probe_file = model_probe_results_dir / f"probe-predictions-layer{layer}.pth"
        probe = torch.load(probe_file, map_location=device)
        probe.eval()

        loader = data.DataLoader(
            data.TensorDataset(reps[:, layer]), batch_size=args.batch_size
        )
        predictions = []
        ...
        assert len(entries) == len(predictions), (len(entries), len(predictions))
