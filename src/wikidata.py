...
def get_occupations(occupation_ids: Sequence[str]) -> Sequence[entity.WikidataItem]:
...
@retry()
def get_entities_with_occupation(occupation_id: str, limit: int = 100) -> Sequence[str]:
...
ORDER BY DESC(?sitelinks)"""
    query += f"LIMIT {limit}"
    results = sparql.return_sparql_query_results(query)
    names = [result["itemLabel"]["value"] for result in results["results"]["bindings"]]
    return tuple(names)


def get_entities_by_occupation(
    occupation_ids: Sequence[str], **kwargs: Any
    ...
    for occupation_id in tqdm(occupation_ids, desc="get entities for each occupation"):
