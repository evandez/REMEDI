...
@pytest.mark.parametrize('path,expected,strict', (
...
def test_maybe_relative_to_repo(root, path, expected, strict):
    ...
    actual = env.maybe_relative_to_repo(path)
    if strict:
...
@pytest.mark.parametrize('name,default,environ,expected,strict', (
    (
        'TEST_ENV_EXISTS',
        'unused',
        {
            'TEST_ENV_EXISTS': '/foo/bar',
            'TEST_ENV_ALSO_EXISTS': 'baz'
        },
        '/foo/bar',
        True,
    ),
...
def test_read_path(root, name, default, environ, expected, strict):
    ...
    actual = env.read_path(name, default)
    if strict:
        assert str(actual) == expected
    else:
        assert str(actual).endswith(f'{root.name}/{expected}')
...
@pytest.mark.parametrize('fn,var,expected', (
    (env.data_dir, env.ENV_DATA_DIR, env.DEFAULT_DATA_DIR),
    (env.models_dir, env.ENV_MODELS_DIR, env.DEFAULT_MODELS_DIR),
    (env.results_dir, env.ENV_RESULTS_DIR, env.DEFAULT_RESULTS_DIR),
))
